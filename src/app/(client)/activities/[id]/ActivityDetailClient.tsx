...
export default function ActivityDetailClient({ id }: ActivityDetailClientProps) {
    ...
    useEffect(() => {
        const loadActivity = async () => {
             setLoading(true);
             try {
                const data = await api.get<Activity>(`/client/activities/${id}`);
                setActivity(data);

                // Load related activities
                const { data: related } = await api.get<{ data: Activity[] }>(`/client/activities?categoryId=${data.categoryId}&limit=3`);
                                ...
                                <Grid size={{ xs: 12, sm: 6, md: 4 }} key={relatedActivity.id}>
                                    ...
                                    >
                                        <CardMedia
                                            component="img"
                                            height="200"
                                            image={getMediaUrl(relatedActivity.thumbnail)}
            ...
            >
                <DialogContent sx={{ position: 'relative', p: 0, overflow: 'hidden' }}>
                    ...
                    >
                        ...
                        <Box
                            component="img"
                            src={getMediaUrl(activity.images[selectedImageIndex])}
