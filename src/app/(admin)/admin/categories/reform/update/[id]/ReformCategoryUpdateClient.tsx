...
export default function ReformCategoryUpdateClient({ id }: Props) {
    ...
    useEffect(() => {
        const loadCategory = async () => {
            try {
                const data = await api.get<{ name: string }>(`/admin/categories/reform/${id}`);
                setName(data.name);
                setLoading(false);
            } catch (err) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            await api.put(`/admin/categories/reform/${id}`, { name: name.trim() });
            ...
            <Container maxWidth="lg">
                <Box sx={{ mb: 4 }}>
                    ...
                    <Typography variant="body2" sx={{ color: '#666', mt: 1 }}>
                        Chỉnh sửa thông tin danh mục thông tin công khai
                    </Typography>
                </Box>
                    ...
                    <form onSubmit={handleSubmit}>
                        <Stack spacing={3}>
                            ...
                            <Stack direction="row" spacing={2} sx={{ mt: 4 }}>
                                ...
                                <Button variant="outlined" onClick={() => router.push('/admin/categories/reform')} disabled={submitting}>
                                    Hủy
                                </Button>
                            </Stack>
                        </Stack>
