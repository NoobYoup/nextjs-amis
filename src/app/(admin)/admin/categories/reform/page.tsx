...
import {
    ...
    DialogTitle, DialogContent, DialogActions, Alert, Container, Stack,
} from '@mui/material';
...
export default function ReformCategoriesPage() {
    ...
    const fetchCategories = useCallback(async () => {
        try {
            const data = await api.get<Category[]>('/admin/categories/reform');
            setCategories(data);
        } catch (err) {
    ...
    const handleOpenDeleteDialog = (id: string) => { setSelectedId(id); setDeleteDialogOpen(true); };
    const handleCloseDeleteDialog = () => { setDeleteDialogOpen(false); setSelectedId(null); };

    const handleConfirmDelete = async () => {
        if (!selectedId) return;
        try {
            await api.delete(`/admin/categories/reform/${selectedId}`);
            fetchCategories();
            ...
            <Container maxWidth="xl">
                <Box sx={{ mb: 4 }}>
                    <Typography variant="h4" sx={{ fontWeight: 700, color: 'var(--foreground)' }}>
                        Quản Lý Danh Mục Công Khai
                    </Typography>
                    <Typography variant="body2" sx={{ color: '#666', mt: 1 }}>
                        Quản lý danh sách các danh mục thông tin công khai của trường
                    </Typography>
                </Box>
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} justifyContent="flex-end" mb={2}>
                    <Button
                        variant="contained"
                        startIcon={<AddIcon />}
                        onClick={() => router.push('/admin/categories/reform/add')}
                    ...
                    <Table>
                        <TableHead sx={{ bgcolor: 'var(--primary-color)' }}>
                            <TableRow>
                                ...
                                <TableCell sx={{ color: '#fff', fontWeight: 600, textAlign: 'center' }}>Hành động</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                                ...
                                <TableRow key={category.id} hover>
                                    ...
                                    <TableCell sx={{ textAlign: 'center' }}>
                                        <IconButton size="small"
                                            onClick={() => router.push(`/admin/categories/reform/update/${category.id}`)}
