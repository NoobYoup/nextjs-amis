...
interface CategoryUpdateClientProps {
    id: string;
}

export default function CategoryUpdateClient({ id }: CategoryUpdateClientProps) {
    ...
    useEffect(() => {
        const loadCategory = async () => {
            try {
                const data = await api.get<{ name: string }>(`/admin/categories/activity/${id}`);
                setName(data.name);
                setLoading(false);
            } catch (err) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.put(`/admin/categories/activity/${id}`, { name: name.trim() });
