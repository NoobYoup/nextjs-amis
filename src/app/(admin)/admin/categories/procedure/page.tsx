...
export default function ProcedureCategoriesPage() {
    ...
    const fetchCategories = useCallback(async () => {
        try {
            const data = await api.get<Category[]>('/admin/categories/procedure');
            setCategories(data);
        } catch (err) {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            await api.delete(`/admin/categories/procedure/${selectedId}`);
            fetchCategories();
            ...
            <Container maxWidth="xl">
                <Box sx={{ mb: 4 }}>
                    ...
                    <Typography variant="body2" sx={{ color: '#666', mt: 1 }}>
                        Quản lý danh sách các danh mục thủ tục hành chính của trường
                    </Typography>
                </Box>
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} justifyContent="flex-end" mb={2}>
                    <Button
                        variant="contained"
                        startIcon={<AddIcon />}
                        onClick={() => router.push('/admin/categories/procedure/add')}
                                ...
                                <TableRow key={category.id} hover>
                                    ...
                                    <TableCell sx={{ textAlign: 'center' }}>
                                        <IconButton size="small"
                                            onClick={() => router.push(`/admin/categories/procedure/update/${category.id}`)}
